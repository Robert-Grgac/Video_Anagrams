...
for i,ref_latent in enumerate(non_deterministic_latent_list):
    for j, prompt in enumerate(prompt_list):
        for alpha in inital_alpha_list:
            video = pipe(
                    # Standard params
                        prompt=prompt,
                    ...
                    #PTM params
                        direct_transfer_steps=60, #Could also play with direct and decay transfer steps
                        decayed_transfer_steps=20,
                        exponent=0.5,
                        initial_alpha=alpha,
                        inital_ref_latent=ref_latent,
                    )
            ...
            export_to_video(frame_list, f"/home/s2710099/outputs/non_deterministic/WanPTDiffusion_{non_deterministic_image_names[i]}_prompt_{j}_alpha_{alpha}.mp4", fps=16)

            print(f"Video saved to /home/s2710099/outputs/non_deterministic/WanPTDiffusion_{non_deterministic_image_names[i]}_prompt_{j}_alpha_{alpha}.mp4")