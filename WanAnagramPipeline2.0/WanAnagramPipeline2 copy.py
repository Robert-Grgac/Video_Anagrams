...
class WanAnagramPipeline2(WanPipeline):
    ...
    def _apply_view_in_pixel_space(
    self,
    ...
    latents_std: torch.Tensor = None,
        ...
        """
        Decode latents to pixel space, apply view transform, encode back.
        
        Args:
            latents:      (B, C, T, H, W) in diffusion-normalized scale
            view:         ViewTransform instance
            forward:      True → view.forward(), False → view.inverse()
            latents_mean: precomputed mean tensor (1, z_dim, 1, 1, 1)
            latents_std:  precomputed 1/std tensor (1, z_dim, 1, 1, 1)
        
        Returns:
            latents back in diffusion-normalized scale after pixel-space transform
        """
        original_dtype = latents.dtype
        
        # 1. Denormalize: diffusion scale → VAE scale
        latents_vae = latents.to(self.vae.dtype)
        latents_vae = latents_vae / latents_std + latents_mean
        
        # 2. Decode to pixel space: (B, C, T, H, W) pixel video
        with torch.no_grad():
            pixels = self.vae.decode(latents_vae, return_dict=False)[0]
        
        # 3. Apply view in pixel space
        if forward:
            pixels = view.forward(pixels)
        else:
            pixels = view.inverse(pixels)
        
        # 4. Encode back to latent space
        with torch.no_grad():
            latents_vae = self.vae.encode(pixels, return_dict=False)
            # WanVAE.encode returns the latent directly (not a distribution)
            if isinstance(latents_vae, tuple):
                latents_vae = latents_vae[0]
        
        # 5. Renormalize: VAE scale → diffusion scale
        latents_out = (latents_vae - latents_mean) * latents_std
        
        return latents_out.to(original_dtype)
    ...
    def __call__(
        ...
        apply_view_in_pixel_space: bool = False,
            ...
            for i, t in enumerate(timesteps):
                ...
                if i > joint_diffusion_steps:
                    if not apply_mean_reduction:
                        if apply_view_in_pixel_space:
                            latents_flipped = self._apply_view_in_pixel_space(latents=latents, view=view, forward=True, latents_mean=latents_mean, latents_std=latents_std)
                        else:
                ...
                if conditional_guidance_scale is not None and negative_guidance_scale is not None:
                    ...
                    if i > joint_diffusion_steps:
                        ...
                        if apply_view_in_pixel_space:
                            noise_cond_b = self._apply_view_in_pixel_space(latents=noise_cond_b, view=view, forward=False, latents_mean=latents_mean, latents_std=latents_std)
                            noise_neg_b = self._apply_view_in_pixel_space(latents=noise_neg_b, view=view, forward=False, latents_mean=latents_mean, latents_std=latents_std)
                            noise_uncond_b = self._apply_view_in_pixel_space(latents=noise_uncond_b, view=view, forward=False, latents_mean=latents_mean, latents_std=latents_std)
                        else:
