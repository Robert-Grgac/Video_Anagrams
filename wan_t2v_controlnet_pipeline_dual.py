...
#
# Sibling of ``wan_t2v_controlnet_pipeline.py`` adapted for two ControlNets,
# one per expert (high-noise / low-noise). Dispatch uses the SAME boundary
# condition that selects the active transformer expert. Beta-003 and beta-004
# use this pipeline for the dual-CN inference variants.
...
def select_controlnet_for_timestep(
    t: torch.Tensor,
    boundary_ratio: Optional[float],
    num_train_timesteps: int,
    controlnet_high,
    controlnet_low,
):
    """Return the controlnet (high or low) for the given timestep tensor.

    Mirrors the transformer-dispatch condition used in the denoising loop:
    high regime when ``boundary_ratio`` is None, otherwise high iff
    ``t >= boundary_ratio * num_train_timesteps``. Lifted to module scope so
    the smoke test can verify dispatch without instantiating the full pipeline.
    """
    if boundary_ratio is None:
        return controlnet_high
    boundary_t = boundary_ratio * num_train_timesteps
    return controlnet_high if t >= boundary_t else controlnet_low


class WanTextToVideoDualControlnetPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    r"""
    Pipeline for text-to-video generation using Wan with TWO ControlNets, one
    per expert. Dispatch uses the same boundary as the transformer expert
    selection: high-noise expert -> ``controlnet_high``,
    low-noise expert -> ``controlnet_low``.
    """

    model_cpu_offload_seq = (
        "text_encoder->transformer->transformer_2->vae->controlnet_high->controlnet_low"
    )
    ...
    def __init__(
        ...
        controlnet_high: WanControlnet,
        controlnet_low: WanControlnet,
    ...
    def __call__(
        ...
        # 6. Encode controlnet frames. Both controlnets share the same canny
        # input; encode once at controlnet_high's dtype/device (they are loaded
        # symmetrically in build_pipeline so dtype matches).
        if (controlnet_latents is None) and (controlnet_frames is not None):
            ...
            controlnet_latents = prepare_controlnet_frames(
                ...
                height,
                width,
                dtype=self.controlnet_high.dtype,
                device=self.controlnet_high.device
            )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # ControlNet dispatch uses the SAME boundary as the transformer
                # dispatch above; routed through the module-level helper so the
                # smoke test exercises the identical condition.
                current_controlnet = select_controlnet_for_timestep(
                    t,
                    self.config.boundary_ratio,
                    self.scheduler.config.num_train_timesteps,
                    self.controlnet_high,
                    self.controlnet_low,
                )
                ...
                if (controlnet_latents is not None) and (controlnet_guidance_start <= current_sampling_percent < controlnet_guidance_end):
                    controlnet_states = current_controlnet(
