"""Smoke test for ``train_beta5.py``.

Validates the batching-on-Blackwell training path before launching a long
run: builds the high-noise transformer + cold-init controlnet, runs a few
effective steps with the real ``micro_batch_size`` + ``gradient_accumulation_steps``,
and asserts the things that have killed past runs:

* attention backend doesn't crash on first forward (the beta-004 Blackwell
  failure mode — flash_attn_func is None at runtime even though the package
  imports cleanly);
* loss finite, GPU mem within tripwire, residual L2 in [0, ceiling);
* transformer carries NO grads, controlnet carries SOME grads;
* EMA step counter advances on optimizer steps, not micro-steps.

Designed to run for a couple of minutes max; the eff_steps default is small
because we just want to validate the loop works end-to-end with the chosen
micro_batch — not to learn anything.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    # Mirror the real-run config: defaults match train_beta5.sbatch.
    p.add_argument("--effective_steps", type=int, default=3)
    p.add_argument("--micro_batch_size", type=int, default=8)
    p.add_argument("--gradient_accumulation_steps", type=int, default=2,
                   help="Smaller than the real run (4) to keep the smoke fast; the loop "
...
def main() -> int:
    ...
    while global_step < target_eff:
        ...
        # Cold init: residual is exactly zero on micro-step 1, non-zero from 2 on.
